...
class ipa(object):
    ...
    def makeReq(self, pdict):
        ...
        if request.status_code > 299:
            ...
            self.log.error('Request Status {0} reason {1}'.format(request.status_code,
                                                                  request.reason))
            raise Exception('Request Status {0} reason {1}'.format(request.status_code,
                                                                  request.reason))
    ...
    def dnsrecord_show(self, dns_zone, record_name):
        m = {'method': 'dnsrecord_show',
             'item': [dns_zone, record_name],
             'params': {
                 'all': True,
             }
        }
    ...
    def dnsrecord_find(self, dns_zone, query_string):
        m = {'method': 'dnsrecord_find',
             'item': [dns_zone, query_string],
             'params': {
                 'all': True,
             }
        }
    ...
    def dnsrecord_del(self, dns_zone, record_name):
        m = {'method': 'dnsrecord_del',
             'item': [dns_zone, record_name],
             'params': {
                 'del_all': True,
             }
    ...
    def dnsrecord_add(self, dns_zone, record_name, ip_address):
        m = {'method': 'dnsrecord_add',
             'item': [dns_zone, record_name],
             'params': {
                 'a_part_ip_address': ip_address,
             }
        }
        results = self.makeReq(m)
    ...
    def is_valid_ipv4_address(address):
        ...
        except AttributeError:  # no inet_option here
            try:
